...
def testRestrictedCodepoints(codepointSeq, glyphName):
    ...
    for c in codepointSeq:
        if c < int('20', 16):
            raise Exception(f"The glyph '{glyphName}' contains a codepoint that is below U+20. You cannot encode glyphs below this number because various typing environments get confused when you do.")

        if c == int('20', 16):
            raise Exception(f"The glyph '{glyphName}' contains U+20. This is space - you shouldn't be using a glyph for this.")

        if c == int('a0', 16):
            raise Exception(f"The glyph '{glyphName}' contains U+a0. This is a space character - you shouldn't be using a glyph for this.")

        if c > int('10FFFF', 16):
            raise Exception(f"The glyph '{glyphName}' contains a codepoint that is above U+10FFFF. The Unicode Standard currently does not support codepoints above this number.")
